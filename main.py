...
class Car:
    def __init__(self):
        self.surface = pygame.image.load("./src/Images/car.png")
...
def run_car(genomes, config):
    ...
    while True:
        ...
        text = generation_font.render(
            f"Generation : {str(generation)}", True, (255, 255, 0)
        )
        text_rect = text.get_rect()
        text_rect.center = (screen_width / 2, 100)
        screen.blit(text, text_rect)

        text = font.render(f"remain cars : {str(remain_cars)}", True, (0, 0, 0))
        text_rect = text.get_rect()
...
if __name__ == "__main__":
    ...
    config_path = "./src/config-feedforward.txt"
